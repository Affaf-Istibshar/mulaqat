...
final List<Country> countriesData = countries;
List<Country>? convertCountryCodesToCountries(List<String>? countryCodes) {
  if (countryCodes == null) return null;

  final List<Country> countries = countryCodes.map((code) {
    final Country? country =
        countriesData.firstWhereOrNull((c) => c.code == code);
    return country ??
        Country(
            name: 'Unknown',
            code: code,
...
class _TelephoneTextState extends State<TelephoneText> {
  ...
  late List<Country> matchingCountries;
  @override
  void initState() {
    ...
    if (widget.initialValue == null || widget.initialValue == '')
      ...
      if (widget.initialValue!.startsWith('+')) {
        ...
        if (countryList.any((element) => fd.startsWith(element.dialCode))) {
          matchingCountries = countryList
              .where((element) => fd.startsWith(element.dialCode))
              .toList(); // Initialize this variable
          for (var r in matchingCountries) {
  ...
  Widget build(BuildContext context) {
    return IntlPhoneField(
      ...
      countries: widget.allowedCountryCodes == null ||
          ...
          ? null
          : convertCountryCodesToCountries(widget.allowedCountryCodes),
      onChanged: (phone) {
        setState(() {
  ...
  void changeAppState() {
    ...
    if (fullNumber == '' || fullNumber.length <= 2) {
      ...
      if (widget.validatorPhoneMustBeValid != null &&
        ...
        if (matchingCountries.any((element) => element.dialCode == coCode)) {
          var el = matchingCountries
              .firstWhere((element) => element.dialCode == coCode);
